...
export default function EducatorDashboard() {
  ...
  return (
    /* Increased max-width to 1800px for wider screens */
    <div className="max-w-[1800px] mx-auto py-4 px-6 lg:px-0">
      
      {/* HEADER SECTION - Scaled up text */}
      <div className="mb-12">
        <h1 className="text-5xl font-black text-slate-800 tracking-tighter">Command Center</h1>
        <p className="text-slate-400 text-xl font-bold mt-2">Welcome back, Prof. Santos. Here's your class overview.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 items-start">
        ...
        <div className="lg:col-span-3 space-y-10">
          ...
          {/* CLASS CARDS GRID - Increased gap and text size */}
          ...
          <div className="bg-white rounded-[3.5rem] border border-slate-100 p-10 lg:p-10 shadow-sm">
            ...
            <div className="space-y-6">
               <StudentRow name="Penduko, Pedro" xp="850" progress={20} modules="1/5" status="AT-RISK" />
               <StudentRow name="Dela Cruz, Juan" xp="2,400" progress={90} modules="4/5" status="COMPLETED" />
               <StudentRow name="Santos, Maria" xp="1,250" progress={45} modules="2/5" status="ON-TRACK" />
            </div>
          </div>
        </div>

        {/* RIGHT SIDEBAR - Sticky for large screens */}
        <div className="space-y-10 lg:sticky lg:top-10">
          <div className="bg-[#0f172a] rounded-[3rem] p-10 text-white shadow-2xl">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <select className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-base font-bold focus:outline-none focus:ring-2 focus:ring-orange-500 appearance-none">
                  <option>Select Module</option>
                  <option>Typhoon Safety</option>
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <input type="date" className="w-full bg-white/5 border border-white/10 rounded-2xl pl-14 pr-5 py-4 text-base font-bold focus:outline-none focus:ring-2 focus:ring-orange-500" />
                </div>
              </div>
          ...
          {/* RESOURCE LIBRARY - Scaled up for wide screens */}
            <div className="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <h3 className="font-black text-slate-800 uppercase text-sm tracking-widest">Resource Library</h3>
                <Plus size={20} className="text-slate-300" />
              </div>
              <div className="space-y-4">
                <LibraryItem icon={<BookOpen size={22}/>} label="Lesson Plans" />
                <LibraryItem icon={<Users size={22}/>} label="Student Reports" />
                <LibraryItem icon={<PieChart size={22}/>} label="Analytics Exports" />
              </div>
            </div>

            {/* QUICK ACTIONS - Scaled up for wide screens */}
            <div className="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-sm">
  ...
  return (
    <div className="bg-white p-10 rounded-[3rem] border-b-[6px] border-orange-500 shadow-xl shadow-slate-200/60 transition-transform hover:-translate-y-2">
      ...
      <div className="grid grid-cols-3 gap-4 text-center">
        <div>
          <p className="text-3xl font-black text-slate-800">{students}</p>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Students</p>
        </div>
        <div>
          <p className="text-3xl font-black text-teal-500">{completion}</p>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Avg. Comp.</p>
        </div>
        <div>
...
function StudentRow({ name, xp, progress, modules, status }) {
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-slate-50 hover:bg-white rounded-[2rem] border border-transparent hover:border-slate-100 transition-all group">
      <div className="flex items-center gap-4 mb-4 md:mb-0">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Last Login: 5 days ago</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        <div className="text-right">
          <p className="text-sm font-black text-slate-800">{xp} XP</p>
        ...
        <div className="flex items-center gap-2">
            {isAtRisk ? (
                <span className="bg-red-50 text-red-600 text-[10px] font-black px-3 py-1 rounded-full flex items-center gap-1">
                    <Info size={12}/> AT-RISK
                </span>
            ) : (
                <span className="bg-teal-50 text-teal-600 text-[10px] font-black px-3 py-1 rounded-full flex items-center gap-1">
...
function LibraryItem({ icon, label }) {
  return (
    <button className="w-full flex items-center justify-between p-4 bg-slate-50 hover:bg-white border border-transparent hover:border-slate-100 rounded-2xl transition-all group text-slate-600 hover:text-orange-500">
      ...
      <div className="bg-white rounded-[3rem] w-full max-w-lg p-12 shadow-2xl relative overflow-hidden">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <input type="text" placeholder="Enter title" className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-orange-500 focus:bg-white rounded-2xl font-bold outline-none transition-all" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Section Name</label>
            <input type="text" placeholder="e.g. Sec C" className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-orange-500 focus:bg-white rounded-2xl font-bold outline-none transition-all" />
          </div>
